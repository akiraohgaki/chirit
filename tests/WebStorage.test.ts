...
Deno.test('WebStorage (mode=local)', { sanitizeResources: false, sanitizeOps: false }, async (t) => {
  await runTestSteps(t, 'local');
});

Deno.test('WebStorage (mode=session)', { sanitizeResources: false, sanitizeOps: false }, async (t) => {
  await runTestSteps(t, 'session');
});

Deno.test('WebStorage (mode=invalidmode)', { sanitizeResources: false, sanitizeOps: false }, async (t) => {
  await t.step('constructor()', () => {
    assertThrows(() => {
      // deno-lint-ignore ban-ts-comment
      // @ts-ignore
      new WebStorage('invalidmode');
    }, Error);
  });
});
